...
async function routeTTS(body) {
  ...
  if (kind === "replicate") {
    const model = String(fields.model || get(S.ttsModel) || "").trim();
...
Hooks.once("init", () => {
  ...
  game.settings.register(NAMESPACE, S.ttsModel, { name: "BoobaStudio: TTS model", hint: "For Replicate, enter the model as owner/name. The selected model is added to Narration's Variant selector.", scope: "client", config: true, type: String, default: "tts-1" });
