...
function replicateHeaders(tokenOverride = "") {
  const configured = String(tokenOverride || get(S.replicateToken) || "").trim();
...
async function routeTTS(body) {
  ...
  if (kind === "replicate") {
    const requestedModel = String(fields.model || "").trim();
    const genericModel = String(get(S.ttsModel) || "").trim();
    const model = String(requestedModel && requestedModel !== "tts-1" ? requestedModel : get(S.ttsReplicateModel) || (genericModel && genericModel !== "tts-1" ? genericModel : "") || (String(get(S.imageProvider) || "").toLowerCase() === "replicate" ? get(S.replicateModel) : "") || "").trim();
    if (!model || !model.includes("/")) return new Response(JSON.stringify({ success: false, error: { message: "Configure a Replicate narration model in owner/name form under BoobaStudio TTS settings." } }), { status: 400, headers: { "Content-Type": "application/json" } });
    const voice = String(fields.voice || fields.voice_id || get(S.ttsVoice) || "").trim();
    const speed = Number(fields.speed || 1);
    const responseFormat = String(fields.response_format || get(S.ttsFormat) || "mp3");
    let configured = {};
    try { configured = JSON.parse(String(get(S.ttsReplicateInput) || "{}")); } catch { return new Response(JSON.stringify({ success: false, error: { message: "Replicate TTS input JSON is not valid JSON." } }), { status: 400, headers: { "Content-Type": "application/json" } }); }
    const defaults = { text: textInput, voice, speed, response_format: responseFormat };
    const values = { ...defaults, input: textInput, prompt: textInput, model };
    const replace = (value) => {
      if (typeof value === "string") return value.replace(/\{\{(text|input|prompt|voice|speed|response_format|model)\}\}/g, (_, name) => String(values[name] ?? ""));
      if (Array.isArray(value)) return value.map(replace);
      if (value && typeof value === "object") return Object.fromEntries(Object.entries(value).map(([name, item]) => [name, replace(item)]));
      return value;
    };
    const configuredInput = replace(configured);
    const input = configuredInput && typeof configuredInput === "object" && !Array.isArray(configuredInput) ? { ...defaults, ...configuredInput } : defaults;
    const modelPath = model.split("/").map((part) => encodeURIComponent(part)).join("/");
    endpoint = `${replicateBaseUrl()}/models/${modelPath}/predictions`;
    const controller = new AbortController();
    const timeout = Math.max(1000, Number(get(S.timeout)) || 120000);
    const timer = setTimeout(() => controller.abort(), timeout);
    try {
      const replicateKey = String(get(S.replicateToken) || get(S.ttsApiKey) || (String(get(S.apiKey) || "").startsWith("r8_") ? get(S.apiKey) : "")).trim();
      const started = await fetch(endpoint, { method: "POST", headers: replicateHeaders(replicateKey), body: JSON.stringify({ input }), signal: controller.signal });
      ...
      if (!started.ok) {
        ...
        return new Response(JSON.stringify({ success: false, error: { message: providerStatusMessage(started.status, detail) } }), { status: started.status, headers: { "Content-Type": "application/json" } });
      }
      ...
      while (current?.status && !["succeeded", "failed", "canceled"].includes(current.status) && Date.now() < deadline) {
        ...
        const status = await fetch(statusUrl, { headers: replicateHeaders(replicateKey), signal: controller.signal });
        current = await status.json().catch(() => null);
        if (!status.ok) return new Response(JSON.stringify({ success: false, error: { message: providerStatusMessage(status.status, String(current?.error || current?.detail || "").trim()) } }), { status: status.status, headers: { "Content-Type": "application/json" } });
      }
      if (current?.status !== "succeeded") return new Response(JSON.stringify({ success: false, error: { message: String(current?.error || `Replicate narration ended with status ${current?.status || "timeout"}`) } }), { status: 502, headers: { "Content-Type": "application/json" } });
      const output = Array.isArray(current.output) ? current.output[0] : current.output;
      const audioUrl = typeof output === "string" ? output : String(output?.audio_url || output?.url || output?.audio || output?.output || "");
      if (!audioUrl) return new Response(JSON.stringify({ success: false, error: { message: "Replicate narration response did not contain an audio URL. Check the model's output and TTS input JSON." } }), { status: 502, headers: { "Content-Type": "application/json" } });
      if (audioUrl.startsWith("data:")) return new Response(JSON.stringify({ success: true, result: audioUrl }), { status: 200, headers: { "Content-Type": "application/json" } });
      const audioResponse = await fetch(audioUrl, { signal: controller.signal });
      if (!audioResponse.ok) return new Response(JSON.stringify({ success: false, error: { message: providerStatusMessage(audioResponse.status, "Replicate audio output could not be downloaded") } }), { status: 502, headers: { "Content-Type": "application/json" } });
      const mime = audioResponse.headers.get("content-type") || `audio/${responseFormat === "mp3" ? "mpeg" : responseFormat}`;
      const encoded = await arrayBufferToBase64(await audioResponse.arrayBuffer());
      return new Response(JSON.stringify({ success: true, result: `data:${mime};base64,${encoded}` }), { status: 200, headers: { "Content-Type": "application/json" } });
    } finally {
      clearTimeout(timer);
    }
...
function localVoiceCatalog() {
  ...
  if (String(get(S.ttsProvider) || "openai").toLowerCase() === "replicate") {
    const voice = String(get(S.ttsVoice) || "").trim();
    return voice ? [{ voice_id: voice, name: voice, description: "Configured Replicate TTS voice", preview_url: "", labels: {} }] : [];
  }
  try {
...
Hooks.once("init", () => {
  ...
  game.settings.register(NAMESPACE, S.ttsProvider, { name: "BoobaStudio: TTS provider", hint: "Use OpenAI, OpenRouter, Replicate, or ElevenLabs for the existing narration and audio workflow.", scope: "client", config: true, type: String, default: "openai", choices: { openai: "OpenAI", openrouter: "OpenRouter", replicate: "Replicate", elevenlabs: "ElevenLabs" } });
  ...
  game.settings.register(NAMESPACE, S.ttsModel, { name: "BoobaStudio: TTS model", hint: "Generic TTS model override. For Replicate, use the dedicated Replicate TTS model setting below.", scope: "client", config: true, type: String, default: "tts-1" });
  game.settings.register(NAMESPACE, S.ttsReplicateModel, { name: "BoobaStudio: Replicate TTS model", hint: "Optional Replicate owner/model for Narration. If blank, BoobaStudio uses a non-default TTS model or the existing Replicate model setting.", scope: "client", config: true, type: String, default: "" });
  ...
  game.settings.register(NAMESPACE, S.ttsReplicateInput, { name: "BoobaStudio: Replicate TTS input JSON", hint: "Optional JSON merged into Replicate narration input. Use {{text}}, {{voice}}, {{speed}}, {{response_format}}, and {{model}} placeholders.", scope: "client", config: true, type: String, default: "{}" });
