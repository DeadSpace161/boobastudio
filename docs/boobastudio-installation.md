...
2. Paste the current manifest URL:
   ...
   `https://raw.githubusercontent.com/DeadSpace161/boobastudio/main/releases/boobastudio-2.2.130-manifest.json`

3. Install **BoobaStudio**, enable it in the world, and restart or reload the world.

The release archive is also available at:

`https://raw.githubusercontent.com/DeadSpace161/boobastudio/main/releases/boobastudio-2.2.130.zip`
...
For Cloudflare AI Gateway, use the native Replicate base URL (without `/compat/chat/completions`): `https://gateway.ai.cloudflare.com/v1/<account-id>/<gateway-name>/replicate`. If **Authenticated Gateway** is enabled, add the Cloudflare Run token to **Provider custom headers JSON** as `{"cf-aig-authorization":"Bearer <cloudflare-run-token>"}`. Keep the Replicate token in the dedicated Replicate API token field.
