...
The current locally packaged release is **2.2.107**. The public test world remains on the published 2.2.97 package; the 2.2.107 manifest and ZIP are independently verified locally and await repository publication.
...
Release 2.2.106 prevents the original thread data-model migration from overwriting arbitrary local provider model IDs. Release 2.2.107 also exposes the configured local music model in the existing song selector. The local provider retains ownership of custom model validation while hosted-mode migration behavior remains unchanged. Package checks, provider tests, build output, and archive integrity pass locally; live verification awaits publication.
