...
Release 2.2.34 is the current package and has been installed and exercised on Foundry v14 Build 363. The existing Journal ProseMirror image action has previously submitted one mocked `POST /v1/images/generations` request through the local provider, produced a non-placeholder preview, and retained one history entry. No Replicate request was made.
...
Live Foundry v14 validation confirmed release 2.2.34 loads as an active module, registers both `imageModel` and `replicateBaseUrl`, and routes an intercepted Flux Fill request to the expected model prediction endpoint with prompt, image, mask, and strength fields. No paid Replicate request was made during this validation.
...
- Foundry v14 Build 363 runtime deployment has been verified through the public test server: BoobaStudio 2.2.34 loads as an active module in `test_sworn`, and the corrected provider adapter is served successfully. A paid browser-driven Replicate generation remains intentionally separate from automated routing validation.
...
- The test server has been validated with BoobaStudio 2.2.34 and the stable entrypoint filename. The fresh-journal page-sheet probe confirms the existing BoobaStudio dropdown and image action render in Foundry v14. A subsequent fully automated fresh-session attempt did not reliably transition from the ProseMirror image menu item into the image prompt window; this remains an interaction-state test gap, not evidence of a provider failure.
