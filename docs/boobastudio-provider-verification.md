...
- The test server is currently running BoobaStudio 2.2.17 with a stable cache-busted entrypoint filename. The hook listeners are present at runtime, but the fresh-journal probe still does not render the custom editor action; the v14 menu construction path remains an open compatibility issue before claiming the journal insertion slice complete.
