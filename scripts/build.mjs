...
const ttsDataMethodPattern = /static async getData\(t,e="tts-1"\)\{.*?\}static async showDialog/;
const ttsDataMethodReplacement = 'static async getData(t,e="tts-1"){let i=["tts-1","tts-1-hd","eleven_v3","eleven_flash_v2_5","eleven_multilingual_v2","eleven_turbo_v2_5","eleven_sound","eleven_music","eleven_sts"],a=M.apiModels?.tts,s=Array.isArray(a)&&a.length>0?[...a]:i,l=String(game.settings.get("boobastudio","ttsProvider")||"openai").toLowerCase(),u=String(game.settings.get("boobastudio","ttsModel")||"").trim()||(l==="openrouter"?"google/gemini-3.1-flash-tts-preview":l==="elevenlabs"?"eleven_multilingual_v2":"tts-1");u&&!s.includes(u)&&s.push(u),e=u;let o=M.getModelsAsHash(s),n=String(game.settings.get("boobastudio","ttsVoice")||"").trim()||(l==="openrouter"?"Enceladus":"onyx");if(!o[u]||l==="openrouter")o[u]={...(o[u]||{}),fields:[{id:"speechcontent",type:"textarea",maxLimit:4096},{id:"voice",type:"select",choices:l==="openrouter"?{Enceladus:"Enceladus"}:{Enceladus:"Enceladus",alloy:"alloy",nova:"nova",onyx:"onyx"},value:n}],maxLength:4096};t.ttsmodels=s.reduce((r,l)=>{let u="boobastudio.ttsmodels."+l,h=game.i18n.localize(u);return r[l]=h===u?l:h,r},{}),t.ttschoice=e,t.ttsmanaHtml=nt(0,3,"",{withTooltip:!0}),await M.prepareTemplates(o[e]||{fields:[]});let r=await _.renderTemplate("modules/boobastudio/templates/song/tts_action.hbs",{});t.modelFields=await _.renderTemplate("modules/boobastudio/templates/modelform.hbs",{fields:o[t.ttschoice]?.fields||[],actionSlot:r})}static async showDialog';
...
const localThreadModelChoices = 'choices:P.chatModels.reduce((o,n)=>(o[n]=n,o),{})';
const localThreadModelChoicesReplacement = 'choices:void 0';
...
const localSceneModelListInsertion = 'l?(r[`${h}::client`]=m,delete r[h]):void 0}if(typeof globalThis.__boobastudioLocalProviderConfigured==="function"&&globalThis.__boobastudioLocalProviderConfigured()){let h=String(game.settings.get("boobastudio",game.settings.get("boobastudio","imageProvider")==="replicate"?"replicateModel":"imageModel")||"").trim();h&&(r[`${h}::client`]??=h)}return r}';
const hostedClientOnlyContext = 'e.clientOnlyModeEnabled=!!game.settings.get("boobastudio","clientOnlyMode")&&typeof globalThis.__boobastudioLocalProviderConfigured==="function"&&globalThis.__boobastudioLocalProviderConfigured()';
const localClientOnlyContext = 'e.clientOnlyModeEnabled=typeof globalThis.__boobastudioLocalProviderConfigured==="function"&&globalThis.__boobastudioLocalProviderConfigured()';
if (!songPatchedEntry.includes(imageModelListInsertion)) throw new Error("Expected image model map signature was not found");
if (!songPatchedEntry.includes(hostedClientOnlyContext)) throw new Error("Expected client-only image access signature was not found");
...
if (!songPatchedEntry.includes(hostedThreadModelChoices)) throw new Error("Expected thread model choices signature was not found");
...
const localGalleryContext = "e.canUseGallery=!!(e.userInfo.alive||typeof globalThis.__boobastudioLocalProviderConfigured===\"function\"&&globalThis.__boobastudioLocalProviderConfigured()),";
const imageNoAccountContext = "e.userInfo.alive||await tr.noAccountContext(e)";
...
if (!versionedEntrySource.includes("google/gemini-3.1-flash-tts-preview") || !versionedEntrySource.includes('l==="openrouter"?{Enceladus')) {
  throw new Error("Versioned entry bundle is missing the provider-aware narration selector");
}
